...
const ChatRoom = () => {
  ...
  const connectWebSocket = () => {
    ...
    const wsUrl = `wss://chat-app-backend-zik2.onrender.com?username=${username}&password=${password}`;
...
const ChatRoom = () => {
  ...
  const connectWebSocket = (retries = 0) => {
    const maxRetries = 5;
    const retryDelay = Math.min(1000 * 2 ** retries, 30000); // Exponential backoff with max delay

    const username = import.meta.env.VITE_WS_USERNAME;
    const password = import.meta.env.VITE_WS_PASSWORD;
    const wsUrl = `wss://chat-app-backend-zik2.onrender.com?username=${username}&password=${password}`;

    console.log("WebSocket Credentials:", username, password); // Debugging
